...
Break each column headers into tuples.  Then, use **`MultiIndex.from_tuples()`** to convert the dataframe into  multiple levels.  The **`.columns`** property is a **MultiIndex** object.
...
Each level (header) is an index object, use **`MultiIndex.get_level_values()`** to the entire header for the desired level. Redundant column names will be observed.
...
**`MultiIndex.levels`** return the unique values of all levels.
...
**`.loc()`** is used for multiIndex selection. DON'T attempt to use `iloc()`
...
Select **specific level 2 columns**  
...
Select specifc level0 and level1 columns
