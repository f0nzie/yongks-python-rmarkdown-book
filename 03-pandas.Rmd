...
Use **`.loc()`** for multiIndex selection. DON'T attempt to use `iloc()`, it is not supported.  
...
test_df = pd.DataFrame([
             [1,2,3,4,5,6],
             [11,12,13,14,15,16],
             [21,22,23,24,25,26]], index=['row1','row2','row3'])
...
**Selecting Level 0 Header**

There are two methods to select specific **level 0** columns.  
First is by specifying column header name. The results is a dataframe with single level header.
...
To return result with two levels header, 'cheat' python that multiple columns are requested using `[columnName]` or `('columnName',All) 

```{python}
All = slice(None)
print( test_df.loc[:,['Female']], '\n\n',
       test_df.loc[:,('Female',All)])
```

**Selecting Level 1 Header**

```{python}
test_df.loc[ : , (All, ['Medium'])]
```

**Selecting Level 0 and Level1 Headers**

Select multiple of level0 and multiple of level1 columns

```{python}
test_df.loc[ : , (['Female','Male'], ['Medium','High'])]
```

Select single of level0 and single of level1 column. The result is a **Series** if using `( , )` notation. If using `( , [ ])` notation, dataframe is returned.
...
Select single of level0 and multiple of level1 columns. 

```{python}
test_df.loc[ : , ('Female', ['Low','High'])]
```
