...
```{python error-missing-label, eval=FALSE}
...
```{python error-pandas-no-attribute, eval=FALSE}
...
```{python error-no-attribute-keys, eval=FALSE}
# error
...
```{python, error-no-attribute-get_dtype, eval=FALSE}
...
```{python, error-no-attribute-get_values, eval=FALSE}
# error
temp_cat.get_values()
```
...
```{python, error-no-attribute-get_values-2, eval=FALSE}
