...
The lexicon is a dictionary. Converting dictionary to list require few steps:  
- Convert dict to dict_items, which is a list containing items, each item is one dict). 
- Convert to dict_items to list

```{python}
list(vadersa.lexicon.items())[:20]  ## convert dict to list of dict
```

**There is only three N-Gram in the lexicon**
